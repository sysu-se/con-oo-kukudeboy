...
我在 `Game` 的历史里存的是盘面快照，也就是 `number[][]`。
...
- 容易保证恢复正确
- 对 9x9 数独来说，快照开销可接受
- 比起回放 move，更不容易在非法输入、givens 保护等场景里出错

### 复制策略

- `grid` / `givens` 都使用二维数组深拷贝
- undo/redo 栈中的每个 snapshot 也深拷贝
- `getGrid()` / `getGivens()` 返回副本
- `getSudoku()` 返回 clone
...
如果误用浅拷贝，不同实例会共享内部行数组，修改一个盘面会污染另一个盘面。
...
- `createSudokuFromJSON(json)` 检查基础结构
- 用 `grid` 和 `givens` 重建 `Sudoku`
...
- adapter 自身的 Svelte store
- UI 临时状态，比如 cursor、modal、dropdown

因为这些不是领域状态，而是界面运行时状态。
...
所以我的结论是：HW1 更像“第一版可测试的领域模型”，但要支撑真实接入，还必须补上更强的领域边界，以及一层面向 Svelte 的 adapter。
...
不过对 9x9 数独来说，这些成本很小，换来的是更稳定的封装和更清晰的响应式边界，所以我认为这个 trade-off 是合理的。
